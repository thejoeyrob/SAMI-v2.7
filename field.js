...
(() => {
  ...
  function backupText() {
    ...
    return at
      ? "Backup requested " + new Date(at).toLocaleString("en-GB")
      : "No backup exported for this project";
  }
  function backupRequested(id) {
    ...
    api?.toast(
      "Backup download requested. Check that the .sami file is in your downloads.",
    );
  }
  async function requestPersistence() {
    if (!navigator.storage?.persist) {
      persistence = "This browser does not offer protected storage";
      return false;
    }
    try {
      const ok = await navigator.storage.persist();
      persistence = ok
        ? "Protected storage granted"
        : "Protection not granted — keep a separate backup";
      return ok;
    } catch {
      persistence = "Storage protection unavailable — keep a separate backup";
      return false;
    }
  }
  async function showDataSafety() {
    ...
    $("#modalBody").onclick = async (event) => {
      ...
      if (action === "import") {
        api.closeModal();
        api.runAction("openProject");
        return;
      }
      button.disabled = true;
      try {
        if (action === "retry") {
          await api.save(true, "Manual retry");
          if ($("#dataSaveStatus"))
            $("#dataSaveStatus").textContent = $("#saveState").textContent;
        }
        if (action === "protect") {
          await requestPersistence();
          if ($("#storageProtection"))
            $("#storageProtection").textContent = persistence;
        }
      } finally {
    ...
    try {
      ...
      if ($("#storageEstimate"))
        $("#storageEstimate").textContent = estimate?.quota
          ? `${(estimate.usage / 1048576).toFixed(1)} MB used of approximately ${(estimate.quota / 1048576).toFixed(0)} MB available to this site.`
          : "Storage estimates are unavailable in this browser.";
      if ($("#storageProtection"))
        $("#storageProtection").textContent = persistence;
    } catch {
      if ($("#storageEstimate"))
        $("#storageEstimate").textContent =
          "Storage estimate unavailable. You can still export a backup.";
    }
  }
  const activeTask = () =>
    !!api?.state.tool || document.body.classList.contains("precision-active");
  async function updateWake() {
    const enabled = pref("sami.wakeLock") === "on";
    const want = enabled && activeTask() && !document.hidden;
    if ((!want || !navigator.wakeLock) && wakeSentinel) {
      const w = wakeSentinel;
      wakeSentinel = null;
      await w.release().catch(() => {});
    }
  ...
  function wakeStatus() {
    const text = !navigator.wakeLock
      ? "Keep awake unavailable in this browser"
      : wakeSentinel
        ? "Screen stays awake while drawing"
        : pref("sami.wakeLock") === "on"
          ? "Keep awake ready · starts while drawing"
          : "Keep screen awake: off";
    const button = $("#wakeLockButton");
    if (button) {
      button.setAttribute(
        "aria-pressed",
        String(pref("sami.wakeLock") === "on"),
      );
      button.setAttribute("aria-label", text);
      button.title = text;
      button.classList.toggle("active", !!wakeSentinel);
    }
  ...
  function applyContrast() {
    ...
    if (/^#[a-f0-9]{6}$/i.test(accent)) {
      ...
      root.style.setProperty(
        "--accent-ink",
        (l + 0.05) / 0.05 >= 4.5 ? "#07120b" : "#ffffff",
      );
      const light =
        ["studioLight", "arcticLight", "paper"].includes(root.dataset.theme) ||
        root.dataset.outdoor === "on";
      const panel = style.getPropertyValue("--panel").trim();
      const p = lum(panel);
      const ratio = (Math.max(l, p) + 0.05) / (Math.min(l, p) + 0.05);
      root.style.setProperty(
        "--accent-text",
        ratio >= 4.5 ? accent : light ? "#145331" : "#b9f4d4",
      );
    }
  }
  function mount(workspace) {
    ...
    if (save) {
      save.setAttribute(
        "aria-label",
        "Save status — project backup and storage",
      );
      save.onclick = showDataSafety;
    }
    const backup = $("#backupButton");
    if (backup) backup.onclick = () => api.exportBackup();
    const mapControls = $(".map-controls");
    if (mapControls) {
      const wake = document.createElement("button");
      wake.id = "wakeLockButton";
      wake.type = "button";
      wake.textContent = "☀";
      wake.onclick = async () => {
        setPref("sami.wakeLock", pref("sami.wakeLock") === "on" ? "off" : "on");
        await updateWake();
        api.toast(wake.title);
      };
      ...
      for (const [id, label] of [
        ...
        if (button) {
          ...
          mapControls.append(button);
        }
      }
    }
    wakeStatus();
    new MutationObserver(updateWake).observe(document.body, {
      attributes: true,
      attributeFilter: ["class"],
    });
    if ($("#drawStatus"))
      new MutationObserver(updateWake).observe($("#drawStatus"), {
        attributes: true,
    ...
    retry.id = "storageWarning";
    retry.type = "button";
    retry.className = "storage-warning";
    retry.hidden = true;
    retry.onclick = showDataSafety;
    $("#app")?.append(retry);
    if (warning) {
      retry.hidden = false;
      retry.textContent = warning + " Review storage";
    }
    window.addEventListener("sami:storage", (e) => {
      retry.hidden = false;
      retry.textContent = e.detail.message + " Review storage";
    });
    window.addEventListener("sami:saved", () => {
      if (warningState === "failed") {
        warning = "";
        warningState = "";
        retry.hidden = true;
      }
  ...
  window.addEventListener("sami:storage", (e) =>
    storageNotice(e.detail.state, e.detail.message),
  );
  window.SAMIField = {
    mount,
    showDataSafety,
